...
const MultiplierBadges = ({ rewards, apys }) => {
  ...
  const badges = useMemo(
    () =>
      multiplierBadges.map((badge) => {
        let isUnlocked =
          rewards.multipliers && rewards.multipliers.map(({ name }) => name).includes(badge.id)
        ...
        <a href={link} target="_blank" rel="noreferrer" key={id}>
          <ToolTip
            label={
              multiplier !== 0
                ? `You ${
                    active ? 'are receiving' : 'do not have'
                  } the ${name} x${multiplier} multiplier`
                : active
                ? 'This bonus is enabled'
                : 'You do not have this bonus enabled'
            }
          >
            <div className={cn(styles.badge, { [styles.badgeActive]: active })}>
              <div className={cn(styles.iconSvg, styles[id])} />
            </div>
          </ToolTip>
        </a>
      ))}
