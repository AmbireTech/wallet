import { Button, Loading, Modal, TextInput, Select, ToolTip, NumberInput } from 'components/common'
...
const GuardarianDepositProviderModal = ({
  relayerURL,
  walletAddress,
  selectedNetwork,
  portfolio,
  ...
  const guardarian = useGuardarian({
    relayerURL,
    selectedNetwork,
    initMode,
  ...
  const selectedTokenInPortfolio = portfolio?.tokens?.find(
    ({ address, symbol }) => address === selectedToken?.address || symbol === selectedToken?.value
  )
  ...
  const getCurrentTokenFromBalance = useCallback(() => {
    if (portfolio.tokens && guardarian?.cryptoCurrencies?.data && guardarian.mode === 'sell') {
      ...
      return portfolio?.tokens?.find((t) =>
        token?.networks?.find(
          (nt) =>
            nt?.token_contract?.toLowerCase() === t?.address?.toLowerCase() ||
            (nt?.network === guardarian.NETWORK_MAPPING[guardarian.network] &&
              t?.address.toLowerCase() === guardarian.NATIVE_ADDRESS &&
              nt?.token_contract === null)
        )
      )
    }
    return {}
  ...
  const validationMsg = useMemo(() => {
    ...
    if (guardarian?.marketInfo?.error || guardarian?.estimateInfo?.error)
      return "Sorry! We couldn't estimate your order! Please try again later!"
    ...
    if (guardarian.mode === 'buy') {
      ...
      if (userAmount < marketMin) return `Minimum amount is ${marketData.min} ${marketData.from}`
      if (userAmount > marketMax) return `Maximum amount is ${marketData.max} ${marketData.from}`
      if (currToken && guardarian?.from && userAmount > currToken.balance)
  ...
  return (
    <Modal size="sm" className={styles.wrapper} title={guardarian.mode === 'buy' ? 'Buy' : 'Sell'}>
      <div className={styles.inputCurrenciesWrapper}>
        ...
        <div className={styles.currenciesRowWrapper}>
          ...
          <div className={styles.currency}>
            {(guardarian.mode === 'buy' && !guardarian?.fiatList?.isLoading) ||
            (guardarian?.mode === 'sell' && !guardarian?.cryptoList?.isLoading) ? (
              <Select
                ...
                items={
                  guardarian.mode === 'buy' ? guardarian.fiatList.data : guardarian.cryptoList.data
                }
          ...
          <ToolTip label="This is expected rate. Guardarian guarantees to pick up the best possible rate on the moment of the exchange">
            <p>
              ...
              <span>
                ...
                guardarian?.estimateInfo?.data &&
                validationMsg === '' ? (
                  <>
                    {1 + guardarian?.estimateInfo?.data?.from_currency} ≈{' '}
                    {guardarian?.estimateInfo?.data?.estimated_exchange_rate}{' '}
                    {guardarian?.estimateInfo?.data?.to_currency}
                  </>
                ) : (
                  <></>
                )}
      ...
      <div className={styles.inputCurrenciesWrapper}>
        ...
        <div className={styles.currenciesRowWrapper}>
          {!guardarian.estimateInfo.isLoading ? (
            ...
            <TextInput value="Info..." disabled className={styles.loadingInput} />
          )}
          <div className={styles.currency}>
            {(guardarian?.mode === 'buy' && !guardarian?.cryptoList?.isLoading) ||
            (guardarian?.mode === 'sell' && !guardarian?.fiatList?.isLoading) ? (
              <Select
                selectInputClassName={styles.selectInput}
                className={styles.select}
                iconClassName={styles.selectIcon}
                ...
                items={
                  guardarian?.mode === 'sell'
                    ? guardarian?.fiatList?.data
                    : guardarian?.cryptoList?.data
                }
