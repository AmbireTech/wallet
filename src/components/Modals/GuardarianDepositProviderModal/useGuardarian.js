...
const useGuardarian = function ({
        ...
        data:
          cryptoCurrencies?.data
            ?.filter((t) =>
              t.networks.find(
                (n) =>
                  n.network === NETWORK_MAPPING[network] &&
                  n.payment_methods.some((pm) => pm.withdrawal_enabled)
              )
            )
            .map((t) => ({
              label: t.ticker,
          ...
          cryptoCurrencies?.data
            ?.filter((t) =>
              t.networks.find(
                (n) =>
                  n.network === NETWORK_MAPPING[network] &&
                  n.payment_methods.some((pm) => pm.deposit_enabled) &&
                  tokens.find(
                    (bt) =>
                      bt?.address?.toLowerCase() === n?.token_contract?.toLowerCase() ||
                      (bt?.address === NATIVE_ADDRESS && n?.token_contract === null)
                  )
              )
            )
            .map((t) => ({
  ...
  const fiatList = useMemo(() => {
    if (mode === 'buy') {
      return {
        data:
          onRampFiats?.data?.map((f) => ({
            label: f.ticker,
            value: f.ticker,
            icon:
              f.logo_url ||
              `https://changenow.io/images/sprite/currencies/${f?.ticker?.toLowerCase()}.svg`
          })) || [],
  ...
  useEffect(() => {
    if (mode === 'buy') {
      ...
      if (cryptoList.data && selectedAsset) {
        const isSelectedAssetExistInCryptoList =
          cryptoList.data &&
          cryptoList.data.length &&
          cryptoList.data.find((i) => i.value === selectedAsset.symbol)
    ...
    fetchGet(FIAT_CURRENCIES_URL)
      ...
      .catch((error) => {
        setOnRampFiats({
          ...
          isLoading: false,
          error,
          message: 'Error while fetching fiat list'
        })
      ...
      .catch(
        (error) =>
          ...
          (setMarketInfo({
            ...
            isLoading: false,
            error,
            message: 'Error while fetching market info'
          }),
  ...
  // Estimate
  useEffect(() => {
    if (!network || !from || !to || !mode || !amount)
      return setEstimateInfo({ data: null, isLoading: false })
      ...
      .catch(
        (error) =>
          ...
          (setEstimateInfo({
            ...
            isLoading: false,
            error,
            message: 'Error while fetching estimate info'
          }),
