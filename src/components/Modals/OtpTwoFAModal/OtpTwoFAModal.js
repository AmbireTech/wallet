import { Modal, Button, TextInput, Loading, ToolTip, CountdownTimer } from 'components/common'
...
const OtpTwoFAModal = ({ relayerURL, selectedAcc, setCacheBreak }) => {
  ...
  const generateQR = useCallback(() => {
    const otpAuth = authenticator.keyuri(selectedAcc.email, 'Ambire Wallet', secret)
  ...
  const sendEmail = async () => {
    ...
    if (!success)
      addToast(
        'Unexpected error. This should never happen, please report this on help.ambire.com',
        { error: true }
      )
  ...
  const verifyOTP = async () => {
    ...
    try {
      ...
      const resp = await fetchPost(`${relayerURL}/identity/${selectedAcc.id}/modify`, {
        otp: hexSecret,
        sig: signatureEthers
      })
  ...
  return (
    ...
    >
      {isTimeIsUp && (
        <div className={styles.timerResetMsg}>Please reopen the modal to reset the session.</div>
      )}
      <CountdownTimer
        seconds={TIMER_IN_SECONDS}
        setTimeIsUp={handleTimeIsUp}
        ...
        <div className={styles.qrCode}>
          ...
          <ToolTip label="Back up the QR code in a secure place. It will allow you to recover your 2FA in case you lose access to the device.">
            <a
              ...
              href={imageURL}
              download="ambire-authenticator-code.png"
            >
              Download QR Code.
            </a>
          </ToolTip>
          <div className={cn(styles.imgMsg, { [styles.showSecret]: showSecret })}>
            {!showSecret && (
              <span className={styles.clickHere}>
                Unable to scan code?{' '}
                <button onClick={() => setShowSecret((prevState) => !prevState)}>
                  Click here.
                </button>
              </span>
            )}
            {showSecret && (
              <>
                <span>Enter this OTP in your app:</span>
                <div>{secret}</div>
              </>
            )}
          </div>
