...
const NetworkFeesModal = ({ relayerURL, selectedNetwork }) => {
  ...
  const url = relayerURL
    ? `${relayerURL}/gasPrice/${selectedNetwork.id}?cacheBreak=${cacheBreak}`
    : null
  ...
  return (
    <Modal
      ...
      title="Current network fees"
    >
      <p className={styles.feesInfo}>
        Network fees are determined on a market principle - if more users are trying to use the
        network, fees are higher. Each network has different fees.
      </p>
