...
const FeesAffordability = ({
  ...
  const getMaxTransferableNative = useCallback(
    (speed) => {
      return new BigNumber(nativeToken?.availableBalance).minus(
        estimatedGasFees.gasFees[speed].signerTransactionsCost
      )
    },
  ...
  return !canCoverGasFees(selectedGasSpeed) &&
    selectableTokensUserInputs.filter((a) => a.selected).length > 0 ? (
    <div className="notification-hollow warning mt-3 mb-3">
      Your Signer Wallet will not have enough fees to pay for the migration. Please transfer a
      maximum of{' '}
      <span
