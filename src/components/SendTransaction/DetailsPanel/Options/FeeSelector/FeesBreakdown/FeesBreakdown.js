  ...
  return (
    <div className={styles.wrapper}>
      <div className={styles.fee}>
        ...
        {!isNaN(baseFeeInUSD) && (
          <p className={styles.feeSide}>~$ {formatFloatTokenAmount(baseFeeInUSD, true, 4)}</p>
        )}
      </div>
      ...
      {!!discount && (
        <div className={cn(styles.fee, styles.discount)}>
          ...
          <p className={styles.feeSide}>~$ {formatFloatTokenAmount(discountInUSD, true, 4)}</p>
        </div>
      )}

      {!!discount && (
        <div className={cn(styles.fee, styles.discount)}>
          <p className={styles.feeSide}>You pay:</p>
          <p className={styles.feeSide}>~$ {formatFloatTokenAmount(feeInUSD, true, 4)}</p>
        </div>
      )}
      {!isGasTankEnabled && !isNaN((feeInUSD / estimation.gasLimit) * savedGas) && (
        <div className={cn(styles.fee, styles.warning)}>
          <p className={styles.feeSide}>Enable Gas Tank to save:</p>
          <p className={styles.feeSide}>
      ...
      {!!isGasTankEnabled && (
        <>
          <div className={cn(styles.fee, styles.discount)}>
            <p className={styles.feeSide}>Gas Tank fee token balance:</p>
            <p className={styles.feeSide}>
          ...
          {!isNaN((feeInUSD / estimation.gasLimit) * savedGas) && (
            <div className={cn(styles.fee, styles.discount)}>
              <p className={styles.feeSide}>Gas Tank saves you:</p>
              <p className={styles.feeSide}>
                $ {formatFloatTokenAmount((feeInUSD / estimation.gasLimit) * savedGas, true, 4)}
              </p>
            </div>
