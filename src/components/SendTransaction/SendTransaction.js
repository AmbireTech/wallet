...
import { getDefaultFeeToken, isTokenEligible, makeBundle, addMockedTxnToBundle } from './helpers'
...
export default function SendTransaction({
  ...
  const eligibleRequests = useMemo(
    () =>
      requests.filter(
        ({ type, chainId, account, txn }) =>
          type === 'eth_sendTransaction' &&
          chainId === network.chainId &&
          account === selectedAcc &&
          txn &&
          (!txn.from || txn.from.toLowerCase() === selectedAcc.toLowerCase())
    ...
    return (
      <div className={styles.wrapper}>
        <h3 className={styles.error}>
          SendTransactions: No account or no requests: should never happen.
        </h3>
      </div>
    )
  return (
...
function SendTransactionWithBundle({
  bundle,
  replaceByDefault,
  mustReplaceNonce,
  network,
  ...
  useEffect(() => {
    ...
    const bundleToEstimate =
      network.id === 'arbitrum' && !currentAccGasTankState.isEnabled
    ...
    const reestimate = () =>
      (relayerURL
        ? bundleToEstimate.estimate({
            relayerURL,
            fetch,
            replacing: !!bundleToEstimate.minFeeInUSDPerGas,
            getNextNonce: true,
