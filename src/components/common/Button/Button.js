...
const Button = ({
  ...
  return (
    <button
      ...
      } ${terniaryGradient ? styles.terniaryGradient : ''} ${danger || red ? styles.danger : ''} ${
        clear ? styles.clear : ''
      } ${border ? styles.border : ''} ${disabled ? styles.disabled : ''} ${
        textOnly ? styles.textOnly : ''
      } ${secondary ? styles.secondary : ''} ${full ? styles.full : ''}`}
      disabled={disabled}
      title={title}
      // used with <form>
      type={type}
