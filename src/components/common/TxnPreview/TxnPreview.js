...
function parseExtendedSummaryItem(item, i, networkDetails, feeAssets) {
  ...
  if (i === 0)
    return (
      <div className={`action ${item.toLowerCase()}`} key={`item-${i}`}>
        {item}
      </div>
    )
    ...
    const foundToken =
      ...
      feeAssets.find(
        (x) =>
          ...
          (!item.symbol || x.symbol.toLowerCase() === item.symbol.toLowerCase())
      )
    return (
          ...
          <Fragment>
            {item.address && (
              <div
                ...
                style={{
                  backgroundImage: `url(${
                    foundToken ? foundToken.icon : getTokenIcon(networkDetails.id, item.address)
                  })`
                }}
              />
            )}
...
export default function TxnPreview({
  ...
  const summary = extendedSummary.map((entry) =>
    Array.isArray(entry)
      ? entry.map((item, i) => parseExtendedSummaryItem(item, i, networkDetails, feeAssets))
      : entry
    ...
    <div className={isFirstFailing ? 'txnPreview firstFailing' : 'txnPreview'}>
      <div className="heading">
        <div className="info" onClick={() => !disableExpand && setExpanded((e) => !e)}>
          <div className="summary-container">
            {!disableExpand && (
              <div className="expandTxn">{isExpanded ? <BsChevronUp /> : <BsChevronDown />}</div>
            )}
            <div className="summary">{summary}</div>
          </div>
          {isFirstFailing && (
            <div className="firstFailingLabel">This is the first failing transaction.</div>
          )}
          {!isFirstFailing && !mined && !isKnown(humanizerInfo, txn, account) && (
            <div className="unknownWarning">
        ...
        <div className="actionIcons">
          {onDismiss ? (
            <ToolTip disabled={!disableDismiss || !disableDismissLabel} label={disableDismissLabel}>
              <div
                className={`dismissTxn ${disableDismiss ? 'disabled' : ''}`}
                onClick={(e) => {
                  e.stopPropagation()
      ...
      {isExpanded ? (
        <div className="advanced">
          ...
          <div>
            <b>
              Value to be sent (<i>value</i>):
            </b>{' '}
            {formatUnits(txn[1] || '0x0', 18)} {getNetworkSymbol(network)}
          </div>
          <div>
            <b>Data:</b> {txn[2]}
          </div>
