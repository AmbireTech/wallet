...
const Networks = ({
  ...
  const { dragStart, dragEnter, dragTarget, handle, target, drop } = useDragAndDrop(
    'value',
    onDropEnd
  )
  ...
  const sortedNetworks = [...allNetworks].sort((a, b) => {
    if (sortType === 'custom' && userSorting?.networks?.items?.length) {
      const sorted =
        userSorting.networks.items.indexOf(a.id) - userSorting.networks.items.indexOf(b.id)
      return sorted
    }
  ...
  const networksItems = sortedNetworks
    .filter((n) => !n.hide)
    .concat(currHideNet)
    .map(({ id, name, icon }) => ({
      label: name,
  ...
  return (
    <Select
      ...
      draggable={!!(sortType === 'custom' && !isMobileScreen)}
      dragEnter={dragEnter}
      drop={drop}
      dragStart={(e, index) => {
