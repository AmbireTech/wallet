...
const Transfer = ({
	...
	if (gasTankDetails?.feeAssetsPerNetwork) {
		eligibleFeeTokens = portfolio.tokens.filter((item) =>
			gasTankDetails?.feeAssetsPerNetwork.some(
	...
	useEffect(() => {
		...
		if (gasTankDetails?.feeAssetsPerNetwork) {
			const gasFeeToken = gasTankDetails.feeAssetsPerNetwork.find(
				(ft) => ft.address.toLowerCase() === selectedAsset.address.toLowerCase()
			);
			if (gasFeeToken?.baseToken) {
				const feeBaseToken = gasTankDetails.feeAssetsPerNetwork.find(
			...
			<div className="panel">
				...
				{portfolio.isCurrNetworkBalanceLoading ? (
					...
					<div className="form">
						...
						/>
						{feeBaseTokenWarning ? (
							<p className="gas-tank-convert-msg">
						...
						{gasTankDetails ? (
							<p className="gas-tank-msg">
								<MdWarning /> {gasTankDetails?.gasTankMsg}
							</p>
						) : (
						...
						{showSWAddressWarning ? (
							<Checkbox
								...
								label={
									<span id="binance-address-warning-label">
						...
						<Button icon={<AiOutlineSend />} disabled={disabled} onClick={sendTx}>
							Send
						</Button>
					</div>
				) : (
