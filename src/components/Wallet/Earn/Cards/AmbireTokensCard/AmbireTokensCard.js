...
const ADDR_ADX_SUPPLY_CONTROLLER = '0xD4628FA47AAe2d0f4c8a204f36c2d93AA8dc31F5'
...
const AmbireTokensCard = ({ networkId, accountId, tokens, rewardsData, addRequest }) => {
  ...
  const walletToken = useMemo(
    () => tokens.find(({ address }) => address === WALLET_TOKEN_ADDRESS),
    [tokens]
  )
  const xWalletToken = useMemo(
    () => tokens.find(({ address }) => address === WALLET_STAKING_ADDRESS),
    [tokens]
  )
  const adexToken = useMemo(
    () => tokens.find(({ address }) => address === ADX_TOKEN_ADDRESS),
    [tokens]
  )
  const adexStakingToken = useMemo(
    () => tokens.find(({ address }) => address === ADX_STAKING_TOKEN_ADDRESS),
    [tokens]
  )
  ...
  const onTokenSelect = useCallback(
    (tokenAddress) => {
          ...
          <>
            <div className="info-message">
              <ToolTip label={unbondToolTipLabelMdg}>
                <span>
                  <b>{msToDaysHours(lockedRemainingTime)}</b> until{' '}
                  {parseFloat(leaveLog.walletValue).toFixed(4)} {selectedToken.label} becomes
      ...
      const apyTooltipMsg = `Annual Percentage Yield${
        selectedToken.label === 'WALLET'
          ? `: IN ADDITION to what you earn in ${selectedToken.label}s`
          : ''
      }`
      setDetails([
        [
  ...
  const handleUnstake = useCallback(
    async ({ bigNumberAmount, isMaxAmount }) => {
      let xWalletAmount
  ...
  useEffect(() => {
    async function init() {
      try {
        ...
        const stakingTokenAddress = isAdxTokenSelected()
          ? ADX_STAKING_TOKEN_ADDRESS
          : WALLET_STAKING_ADDRESS
        const stakingPoolInterface = isAdxTokenSelected()
          ? ADX_STAKING_POOL_INTERFACE
        ...
        const [leaveLogs, withdrawLogs] = await Promise.all([
          ...
          provider.getLogs({
            fromBlock: 0,
            ...stakingTokenContract.filters.LogWithdraw(accountId, null, null, null, null)
          })
        ])
        ...
        if (leavePendingToUnlockOrReadyToWithdraw) {
          const { maxTokens, shares, unlocksAt, walletValue } =
