...
const YEARN_DETAILS = {
  ...
  description:
    'Yearn.finance is a suite of products in DeFi that provides yield generation, lending aggregation, and more on the blockchain. The protocol is maintained by various independent developers and is governed by YFI holders.'
}
...
const YearnTesseractCard = ({ networkId, accountId, tokens, addRequest }) => {
  ...
  const isDepositsDisabled = networkId === 'polygon'
  ...
  const { icon, loadVaults, tokensItems, details, onTokenSelect } = useMemo(() => yearn, [yearn])
  ...
  const onValidate = async (type, value, amount) => {
    ...
    if (type === 'Deposit') {
      try {
        ...
        const provider =
          networkId === 'ethereum' ? rpcProviders['ethereum-ambire-earn'] : getProvider(networkId)
  ...
  return (
    <Card
      ...
      moreDetails={
        <EarnDetailsModal title={moreDetails.title} description={moreDetails.description} />
      }
    />
  )
}
