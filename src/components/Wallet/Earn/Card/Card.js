...
const Card = ({
  ...
  const isAmountTooBig =
    parseFloat(amount) >
    (currentToken && 'balanceRaw' in currentToken
      ? parseFloat(utils.formatUnits(currentToken.balanceRaw, currentToken.decimals))
      : 0)
  ...
  const buttonDisabled =
    ...
    parseFloat(amount) <= 0 ||
    isAmountTooBig ||
    (segment === segments[0].value && isDepositsDisabled)
  ...
  const sortedTokenItems = useMemo(
    () =>
      [...tokensItems].sort(
        (a, b) => b?.balance + getEquToken(b)?.balance - (a?.balance + getEquToken(a)?.balance)
      ),
  ...
  const amountLabel = (
    <div className="amount-label">
      Available Amount: <span title={availableAmount}>{availableAmount}</span>
    </div>
  )
  ...
  return (
    <div className="card">
      ...
      {loading ? (
        ...
        <div className="unavailable">Unavailable on this Network</div>
      ) : (
        <div className="content">
          ...
          />
          {customInfo ? (
            <div className="info">{customInfo}</div>
          ) : (
            <>
              <NumberInput
          ...
          <div className="separator" />
          {!!moreDetails && (
            <Button clear icon={<MdOutlineInfo />} onClick={() => showMoreDetails()}>
              See more details
            </Button>
          )}
        </div>
