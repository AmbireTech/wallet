...
const useMovr = () => {
    ...
    const fetchQuotes = useCallback(async (identity, fromAsset, fromChainId, toAsset, toChainId, amount, excludeBridges, sort = 'output') => {
        return fetchSocket(`/quote?fromChainId=${fromChainId}&fromTokenAddress=${fromAsset}&toChainId=${toChainId}&toTokenAddress=${toAsset}&fromAmount=${amount}${excludeBridges.map(b => `&excludeBridges=${b}`)}&sort=${sort}&userAddress=${identity}&recipient=${identity}&singleTxOnly=true&isContractCall=true&bridgeWithGas=false`)
    }, [])
    ...
    const approvalBuildTx = useCallback(async (chainID, owner, allowanceTarget, tokenAddress, amount) => {
        return fetchSocket(`/approval/build-tx?chainID=${chainID}&owner=${owner}&allowanceTarget=${allowanceTarget}&tokenAddress=${tokenAddress}&amount=${amount}`)
    }, [])
    ...
    const checkTxStatus = useCallback(async (transactionHash, fromChainId, toChainId) => {
        return fetchSocket(`/bridge-status?transactionHash=${transactionHash}&fromChainId=${fromChainId}&toChainId=${toChainId}`)
    }, [])
