  ...
  const signersFragment = relayerURL ? (
    <div>
      <Alert
        ...
        text={
          <>
            Signer settings are network-specific. You are currently looking at and modifying the
            signers on {selectedNetwork.name}.{' '}
            <a
      ...
      <Panel className={styles.panel} title="Authorized signers" titleClassName={styles.title}>
        ...
        {errMsg && <h3 className={styles.error}>Error getting authorized signers: {errMsg}</h3>}
        {!showLoading && privilegesList.length ? (
          <Signers
    ...
    <Panel className={styles.panel} title="Authorized signers">
      <h3 className={styles.error}>Unsupported: not connected to a relayer.</h3>
    </Panel>
  )
  return (
    <OfflineWrapper>
