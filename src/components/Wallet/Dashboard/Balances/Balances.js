...
const Balances = ({
  ...
  const otherBalances = portfolio.otherBalances.filter(
  ...
  const gasTankBalances =
    balancesRes &&
    balancesRes.length &&
    balancesRes.map(({ balanceInUSD }) => balanceInUSD).reduce((a, b) => a + b, 0)
  const [truncated, decimals] = gasTankBalances
    ? Number(gasTankBalances.toString()).toFixed(2).split('.')
    : [0, 0]
  const gasTankDetails = {
    label: 'Gas Tank',
    total: {
      full: gasTankBalances || 0.0,
      decimals,
      truncated
    }
  }
      ...
      {portfolio.isCurrNetworkBalanceLoading ? (
        ...
        <div className={styles.otherBalances} ref={otherBalancesRef} onScroll={handleSetBlur}>
          {!otherBalances.length && otherBalancesLoading ? (
            <div className={styles.loadingOtherBalancesWrapper}>
              <Loading />
            </div>
...
const areEqual = (prevProps, nextProps) => {
  return (
    prevProps.selectedNetwork.id === nextProps.selectedNetwork.id &&
    ...
    prevProps.portfolio.isCurrNetworkBalanceLoading ===
      nextProps.portfolio.isCurrNetworkBalanceLoading
  )
}
