...
const Heading = ({ network, relayerURL, portfolio, account, gasTankState, setGasTankState }) => {
  ...
  const gasTankBalancesFormatted = gasTankBalances
    ? formatFloatTokenAmount(gasTankBalances, true, 2)
    : '0.00'
  const totalSaved =
    totalSavedResult &&
  ...
  const currentAccGasTankState = gasTankState.length
    ...
    : setGasTankState([...gasTankState, { account, isEnabled: false }])
  const toggleGasTank = () => {
  ...
  const openGasTankBalanceByTokensModal = () => {
    showModal(
      <GasTankBalanceByTokensModal data={balancesRes && balancesRes.length ? balancesRes : []} />
    )
  }
    ...
    <div className={styles.wrapper}>
      <div className={styles.allNetworksAndToggle}>
        <div
          ...
          onClick={openGasTankBalanceByTokensModal}
        ...
        >
          ...
          {!isLoading ? (
            <h3
              className={cn(styles.bigText, {
                [styles.small]: gasTankBalancesFormatted.length > 6
              })}
            >
              <span>$</span>
              {gasTankBalancesFormatted}
            </h3>
          ) : (
            <Loading />
          )}
