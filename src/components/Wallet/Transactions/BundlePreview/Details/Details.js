...
const getCashback = (bundle, feeTokenDetails) => {
	if (bundle.gasTankFee && bundle.gasTankFee.cashback && feeTokenDetails) {
		return (
			formatUnits(bundle.gasTankFee.cashback.toString(), feeTokenDetails?.decimals).toString() * feeTokenDetails?.price
		)
	}
		...
		<div className={styles.wrapper}>
			...
			{bundle.gasTankFee && feeTokenDetails !== null && mined && (
                ...
                You saved $ ${formatFloatTokenAmount(bundle.feeInUSDPerGas * savedGas, true, 6)}, ${
								cashback > 0 ? `and got back $ ${formatFloatTokenAmount(cashback, true, 6)} as cashback,` : ''
              ...
              `}>
							<DetailsItem
								...
								text={`$ ${formatFloatTokenAmount(bundle.feeInUSDPerGas * bundle.gasLimit - cashback, true, 6)}`}
							/>
						</ToolTip>
					)}
					{savedGas && (
              ...
              `}>
							<DetailsItem title="Total Saved" text={`$ ${formatFloatTokenAmount(totalSaved, true, 6)}`} />
						</ToolTip>
					)}
				</>
			)}
			...
			{bundle.replacesTxId ? <DetailsItem title="Replaces transaction" text={bundle.replacesTxId} /> : null}
			{bundle.txId ? (
				<DetailsItem
