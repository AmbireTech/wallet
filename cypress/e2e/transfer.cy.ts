...
describe('Transfering funds', () => {
  ...
  it('Sends funds via Quick account', () => {
    ...
    cy.get('[data-testid="amount"]').clear().type('0.0000001', { delay: 100 })
    ...
    cy.get('[data-testid="password"]').type(Cypress.env('ACCOUNT_PASSWORD'), { delay: 100 })
    ...
    cy.task('get-confirm-code').then(code => {
      cy.get('[data-testid="confirmationCode"]').type(code, { delay: 100 })
